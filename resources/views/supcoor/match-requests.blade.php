                ...
                <div class="space-y-4">
                    @foreach($acceptedRequests as $request)
                        @php
                            ...
                            // For support coordinators, show the matched participant and indicate which participant they're matching for
                            if ($request->sender_user_id == $currentUser->id) {
                                // Support coordinator sent the request, show receiver participant
                                $participantCode = $receiverParticipantCode ?? 'N/A';
                                $participantId = $request->receiverParticipant->id ?? null;
                                $matchForName = $senderParticipantName;
                            } else {
                                // Support coordinator received the request, show sender participant
                        ...
                        <div class="bg-[#f8f1e1] rounded-lg border border-[#e1e7dd] p-6 hover:shadow-md transition-shadow duration-200">
                            <div class="flex items-start justify-between">
                                ...
                                <div class="flex space-x-2 ml-4">
                                    ...
                                    @endif
                                    <a href="{{ route('sc.messages.index') }}" class="px-4 py-2 bg-[#33595a] text-white rounded-md hover:bg-[#2C494A] transition-colors duration-200 text-sm font-medium">
                                        Start Conversation
                                    </a>
                                </div>
                            </div>
            ...
            if (p && p.participant_code_name) {
                content.innerHTML = `
                    <div class="space-y-6">
                        ...
                        ` : ''}
                        ${p.housemate_preferences ? `
                        <div class="bg-[#f8f1e1] p-4 rounded-lg">
                            <h5 class="font-semibold text-[#3e4732] mb-3">Housemate Preferences</h5>
                            <p class="text-[#2C494A] font-medium text-sm leading-relaxed">${p.housemate_preferences}</p>
                        </div>
                        ` : ''}
                        ${p.communication_preferences ? `
                        <div class="bg-[#f8f1e1] p-4 rounded-lg">
                            <h5 class="font-semibold text-[#3e4732] mb-3">Communication Preferences</h5>
                            <p class="text-[#2C494A] font-medium text-sm leading-relaxed">${p.communication_preferences}</p>
                        </div>
                        ` : ''}
                    </div>
                `;
